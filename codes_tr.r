...
chartSeries(ENV.STOCK$THYAO.IS, 
            type = "candlestick", 
...
chartSeries(ENV.STOCK$EREGL.IS, 
            type = "candlestick", 
            theme = chartTheme("white"))
addSMA(n = 15, col = "red")
addVolatility()
...
chart_Series(Cl(ENV.STOCK$THYAO.IS))
add_TA(Cl(ENV.STOCK$EREGL.IS), on = 1)
add_RSI()

normalise_series <- function(xdat) xdat / coredata(xdat)[1]
mytheme <- chart_theme()
chart_Series(normalise_series(Cl(ENV.STOCK$THYAO.IS)) - 1, theme = mytheme)
add_TA(normalise_series(Cl(ENV.STOCK$EREGL.IS)) - 1, on = 1, col = "red", lty = 1)
...
adf.test(XTS.DRETURNS$CCOLA.IS)
fit1<- auto.arima(XTS.ADJUSTED$CCOLA.IS.Adjusted)
forecast(fit1) %>% plot


fit1<- auto.arima(XTS.DRETURNS$AKBNK.IS)
forecast(fit1) %>% plot
